...
GenXML()
{
	msg1="$1"
	msg2="$2"
	cat << EOF > $TMP_RESULTFILE
<?xml version="1.0"?>
<result outcome="$msg1" security="$TMP_RESULTFILE">$msg2</result>
EOF
	sent_xml=1
}
...
# Create feedback and report directories
mkdir -p "$TMP_FEEDBACKDIR"
mkdir -p "$REPORTDIR"
...
REPORT Starting: "$VALIDATOR $JUDGEIN $JUDGEANS $TMP_FEEDBACKDIR > $INFIFO < $OUTFIFO &"
$VALIDATOR $JUDGEIN $JUDGEANS $TMP_FEEDBACKDIR > $INFIFO < $OUTFIFO &
intv_pid=$!
